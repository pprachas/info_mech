...
def rectangle_mesh(L,H,n_nodes, f_name):
    gmsh.initialize()
    ...
    # add points
    p_rec = []
    for ii in range(len(p_rec_np)):
        p_rec.append(gmsh.model.occ.addPoint(p_rec_np[ii][0],p_rec_np[ii][1], 0,-1))
    
    # add lines
    lines = []
    for ii in range(len(p_rec_np)):
        lines.append(gmsh.model.occ.addLine(p_rec[ii-1],p_rec[ii]))
    # Construct surface:
    ...
    gmsh.model.mesh.setTransfiniteCurve(lines[0],n_nodes)
    gmsh.model.mesh.setTransfiniteCurve(lines[2],n_nodes)
    gmsh.model.mesh.setTransfiniteCurve(lines[1],int(n_nodes*rat)+1)
    gmsh.model.mesh.setTransfiniteCurve(lines[3],int(n_nodes*rat)+1)
...
def plate_hole_mesh(L,H,R, n_nodes,f_name):
    gmsh.initialize()
    rat = (H/2-R)/(H/2)
