...
a_max = 100  # Normalization factor for coordinates
...
# ------------------------- Run Selection -------------------------#
sensor_indices, sensor_signals, cmi_history = greedy_sensor_selection(num_sensors, sigma_y, a_scaled, scaler)

print(len(cmi_history), sensor_indices)

# Save sensor indices
np.savetxt(f'sensor_loc.txt', np.array(sensor_indices))
# ------------------------- Plot CMI Maps -------------------------#
x_grid, y_grid = np.meshgrid(x/a_max, y/a_max)
fig, axes = plt.subplots(1, num_sensors+1, figsize=(3 * (num_sensors+1), 3))
fig.suptitle(f'Uniform load')
# if num_sensors == 1:
...
# Shared colorbar
plt.tight_layout()
plt.savefig(f'all_cmi_maps.pdf')

plt.figure()
plt.semilogx(y,cmi_history[0][:,0]) # sanity check
plt.show()
plt.show()
