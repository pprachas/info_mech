...
coord = [0,10]
x_idx = np.abs(x-coord[0]).argmin()
y_idx = np.abs(y-coord[1]).argmin()
...
scaler.fit(sigma_y[x_idx,y_idx,:,None])
sigma_og1_scaled = scaler.transform(sigma_y[x_idx,y_idx,:,None])
sigma_og1_scaled[np.isclose(sigma_y[x_idx,y_idx,:,None]-scaler.mean_, 0, atol=1e-12)]=0
...
cmi = mi2_grid-mi_grid[y_idx,x_idx] # conditional mutual information
...
plt.colorbar(contour,ax=ax)
plt.plot(x_grid,y_grid,marker = '.', ls = 'none', c='k', markersize=1.0)
plt.plot(x_grid[y_idx,x_idx],y_grid[y_idx,x_idx],marker = 'X', ls = 'none', c='blue', markersize=10)
plt.tight_layout()
...
cmi2 = mi3_grid-mi2_grid #mi3_grid-cmi-mi_grid[y_idx,x_idx] # conditional mutual information
